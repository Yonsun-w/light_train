...
class TxtTrueFiletoGrid(object):
    def __init__(self, config_dict):
        ...
        while (dataTime <= self.end_time):
            truthfilepath = os.path.join(self.config_dict['TruthFileDir'],  dataTime.strftime('%Y_%m_%d') + '.txt')
    ...
    def all_trueFile_txt_to_png(self):
        ...
        for time in self.time_slot:
            truthfilepath =  os.path.join(self.config_dict['TruthFileDir'],  time.strftime('%Y_%m_%d') + '.txt')
            if not os.path.exists(truthfilepath):
                continue
            output_file = os.path.join(self.config_dict['output'],'' + time.strftime('%Y_%m_%d_%H_%M') + '.png')
    ...
    def all_trueFile_txt_to_npy(self):
        ...
        for time in self.time_slot:
            ...
            if not os.path.exists(truthfilepath):
                continue
            output_file = os.path.join(self.config_dict['output'],'' + time.strftime('%Y_%m_%d_%H_%M') + '.npy')
            grid = self.txt_to_grid(truthfilepath,time)
    ...
    def all_trueFile_txt_to_nc(self):
        ...
        for time in self.time_slot:
            ...
            output_file = os.path.join(self.config_dict['output'],
                                           '' + time.strftime('%Y_%m_%d_%H_%M') + '.nc')
            grid = self.txt_to_grid(truthfilepath, time)
        ...
        with open(tFilePath, 'r', encoding='GBK') as tfile:
            for line in tfile:
                ...
                if  t1 <= temp_dt <= t2 and self.check_in_area(lat, lon):
                    ...
                    # 按照范围绘制
                    cur_grid = self.draw_grid_all(cur_grid, x, y)
    ...
    def create_nc(self, output_path, cur_grid):
        ...
        if self.config_dict['equal_dis']:
            f_w.FileOrigins = 'equidistant'
        else :
            f_w.FileOrigins = 'equal_lat&lon'
        ...
        data = datetime.datetime.strptime(self.config_dict['startTime'], "%Y%m%d%H%M%S")
        start = datetime.datetime.strftime(data, "%Y%m%d_%H%M%S")
        data = datetime.datetime.strptime(self.config_dict['endTime'], "%Y%m%d%H%M%S")
...
if __name__ == "__main__":
    nc_path = 'npy/_2015_05_17_00_00.nc'
    f_w = nc.Dataset(nc_path, 'r', format='NETCDF4')  # 创建一个格式为.nc的
    print(f_w.variables['Flash_pre'].shape)
