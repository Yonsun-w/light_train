...
def getHoursGridFromSmallNC_npy(filepath, delta_hour, forecastHourNum):  # 20200619
    ...
    for s in param_list:
        ...
        npy_grid = npy_grid[delta_hour:delta_hour + forecastHourNum ]
        if s == 'RAINNC':
...
class DataGenerator(py_Dataset):
    ...
    def __data_generation(self, datetime_peroid):
        ...
        obs_batch = np.zeros(shape=[self.config_dict['TruthHistoryHourNum'], m, n, 1], dtype=np.float32)
        ...
        nc_grid = getHoursGridFromSmallNC_npy(npyFilepath, delta_hour, self.config_dict['ForecastHourNum'])
        ...
        for hour_plus in range(self.config_dict['TruthHistoryHourNum']):
            ...
            truth_grid = np.load(tFilePath)
            truth_grid = truth_grid.reshape(m, n)
            obs_batch[hour_plus, :, :, :] = truth_grid[:, :, np.newaxis]




        # 这一部分的作用是，获取与历史观测数据等长的wrf
        # 这里获取的是历史3小时
        history_wrf_batch = np.zeros(shape=[self.config_dict['TruthHistoryHourNum'], m, n, self.config_dict['WRFChannelNum']], dtype=np.float32)

        ddt = datetime.datetime.strptime(datetime_peroid, '%Y%m%d%H%M') + datetime.timedelta(hours=-self.config_dict['TruthHistoryHourNum'])
        # read WRF
        # UTC是世界时
        utc = ddt + datetime.timedelta(hours=-8)
        ft = utc + datetime.timedelta(hours=(-6))
        ...
        # 由于wrf(也就是nc格式)存了好几个矩阵，所以一个nc文件转成了好几个npy，这里一个wrf对应的
        # 是一个文件夹  '/data/wenjiahua/light_data/ADSNet_testdata/WRF_data/20200831'
        nc_grid = getHoursGridFromSmallNC_npy(npyFilepath, delta_hour, self.config_dict['TruthHistoryHourNum'])

        history_wrf_batch[:, :, :, 0:self.config_dict['WRFChannelNum']] = nc_grid



        return [wrf_batch, obs_batch, history_wrf_batch] , label_batch
