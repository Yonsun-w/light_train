...
class DCNADSNet_lite_tf_Model(nn.Module):
    def __init__(self, obs_tra_frames, obs_channels, wrf_tra_frames, wrf_channels, config_dict):
        ...
        self.transformer = Transformer(channels=8, layers=1, nhead=1)
    ...
    def forward(self, wrf, obs):
        ...
        c = self.encoder_c(c)
        for t in range(self.wrf_tra_frames):
            wrf_tf = self.transformer(wrf_info_h[t], wrf_encoder[t])
