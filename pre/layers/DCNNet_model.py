...
class Encoder_wrf_model(nn.Module):
    def __init__(self, wrf_tra_frames, wrf_channels, config_dict):
        ...
        self.CNN_module2 = nn.Sequential(
            ...
            nn.MaxPool2d(kernel_size=2, stride=2),
            nn.Conv2d(wrf_channels, 32, kernel_size=1, stride=1),
            # nn.ReLU(),
        )
        self.decoder = nn.Sequential(
            ModulatedDeformConvPack(32, 64, kernel_size=5, stride=1, padding=2),
            nn.ReLU(),
        )
    ...
    def forward(self, wrf):
        ...
        mn = (self.config_dict['GridRowColNum'] // 2) // 2
        wrf_h = torch.zeros([batch_size, 32, mn, mn], dtype=torch.float32).to(wrf.device)
        wrf_c = torch.zeros([batch_size, 32, mn, mn], dtype=torch.float32).to(wrf.device)

        for t in range(self.wrf_tra_frames):
            pre = self.CNN_module2(wrf[t])
            pre = self.decoder(pre)
            wrf_h, wrf_c = self.wrf_encoder_convLSTM2D(pre, wrf_h, wrf_c)
        return wrf_h, wrf_c
