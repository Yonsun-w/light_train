...
class DCNADSNet_attn_Model(nn.Module):
    def __init__(self, obs_tra_frames, obs_channels, wrf_tra_frames, wrf_channels, config_dict):
        ...
        self.encoder_ConvLSTM = ConvLSTM2D_Attn(4, 8, kernel_size=5, img_rowcol=(config_dict['GridRowColNum']//2)//2)
        ...
        self.decoder_ConvLSTM = ConvLSTM2D_Attn(32, 32, kernel_size=5, img_rowcol=(config_dict['GridRowColNum']//2)//2)
        self.CNN_module3 = nn.Sequential(
            ModulatedDeformConvPack(32, 128, kernel_size=5, stride=1, padding=2),
            nn.ReLU(),
    ...
    def forward(self, wrf, obs):
        ...
        for t in range(self.wrf_tra_frames):
            ...
            pre = self.CNN_module3(h.contiguous())
            pre_frames[t] = pre
